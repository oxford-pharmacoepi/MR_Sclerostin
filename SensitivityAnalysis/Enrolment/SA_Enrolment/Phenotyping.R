...
for (i in 1:6){
  source(here("SensitivityAnalysis","Enrolment","SA_Enrolment","PhenotypingHES.R"))
  hes_data <- PhenotypingHes(outc[i],hes,hesD)
  
  source(here("SensitivityAnalysis","Enrolment","SA_Enrolment","PhenotypingGP.R"))
  gp_data <- PhenotypingGP(outc[i],gp,pop,ukb)
  
  source(here("SensitivityAnalysis","Enrolment","SA_Enrolment","PhenotypingUKB.R"))
  ...
  write.csv(t,here("SensitivityAnalysis","Enrolment","SA_Enrolment",paste0("Phenotype_",outc[i],".csv")))
}
