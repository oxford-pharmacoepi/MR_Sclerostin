...
ylabel("MR effect size per SD change in eBMD","Interpreter","latex")
print(f,"Fig6.png","-dpng","-r600")
%%
clear all; close all
path = pwd; path = path(1:end-length('\Figures'));
...
plot(linspace(ax.XLim(1),ax.XLim(2),10),t.Estimate(1)*linspace(ax.XLim(1),ax.XLim(2),10),'--',"Color",'k')
text(-0.1,0.26,'B)','FontSize',17,'Interpreter','latex')
xlabel("MR effect size per SD change in sclerostin levels","Interpreter","latex")
ylabel("MR effect size per SD change in hip fracture","Interpreter","latex")
print(f,"Fig6.1.png","-dpng","-r600")